    ...
    const loadUser = async () => {
        try {
            ...
            if (result.data.avatar) {
                setPreview(`http://localhost:8080/images/UserAvatar/${result.data.avatar}`);
            }
        } catch (error) {
                ...
                <div className="w-full max-w-lg bg-white border rounded-lg p-8 mt-8 shadow-xl">
                    ...
                    <form onSubmit={onSubmit}>
                        ...
                        <div className="mb-6">
                            ...
                            <input type="text" id="username" className="form-input mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 text-lg p-3 bg-gray-100" placeholder="Enter your username" name="username" value={user.username} onChange={onInputChange} />
                        </div>
                        <div className="mb-6">
                            <label htmlFor="email" className="block text-gray-700 font-medium mb-2">Email</label>
                            <input type="email" id="email" className="form-input mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 text-lg p-3 bg-gray-100" placeholder="Enter your email" name="email" value={user.email} onChange={onInputChange} />
                        </div>
                        <div className="mb-6">
